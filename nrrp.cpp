...
unsigned int _paralelInstanceCount = 1;
...
unsigned int _FirstAbsenceDay;
unsigned int* _realAbsenceArray;
unsigned int* _matrixRS;
...
void kerlen_run2(unsigned int* shiftList,unsigned int* shiftMap, unsigned int* currentRoster, unsigned int* penalityFunctions, unsigned int* nonAssignedShifts, unsigned int* staffGaps)
{
	unsigned int i = 0;
	unsigned int j = 0;
	...
	index = (unsigned int*)calloc(_paralelInstanceCount, sizeof(unsigned int));
	firstPhase = (unsigned int*)calloc(_paralelInstanceCount, sizeof(unsigned int));
	...
	while(1)
	{
		for(numInstance = 0; numInstance < _paralelInstanceCount; numInstance++)
		{
			...
			if((index[numInstance] == _dayCount)||infeasible[numInstance]||changesCount[numInstance]>_shiftChangeMaximum)
			{
				...
				for(i = 0; i < _nurseCount; i++)
					nurseInitialize(currentRoster+2*offset, shiftMap+offset, shiftList+_shiftToReroster*numInstance, nonAssignedShifts+offset, staffGaps+_dayCount*3*numInstance, localSearch, probability, i, 0);
				index[numInstance] = 0;
...
void kerlen_run(unsigned int* shiftList,unsigned int* shiftMap, unsigned int* currentRoster, unsigned int* penalityFunctions, unsigned int* nonAssignedShifts, unsigned int* staffGaps)
{
	...
	struct timeb currentTime;
	struct timeb endTime;
	unsigned int* keepShiftList = (unsigned int*)malloc(_paralelInstanceCount*sizeof(unsigned int));
	unsigned int* backTrackCounter  = (unsigned int*)malloc(_paralelInstanceCount*sizeof(unsigned int));
	...
	index = (unsigned int*)calloc(_paralelInstanceCount, sizeof(unsigned int));
	firstPhase = (unsigned int*)malloc(_paralelInstanceCount* sizeof(unsigned int));

	for(i = 0; i < _paralelInstanceCount; i++)
	{
		firstPhase[i] = 1;
		backTrackCounter[i] = 0;
		keepShiftList[i] = 0;
	}
	...
	while(1)
	{
		for(numInstance = 0; numInstance < _paralelInstanceCount; numInstance++)
		{
			...
			if((index[numInstance] == _dayCount)||infeasible[numInstance]||changesCount[numInstance]>_shiftChangeMaximum)
			{
				...
				index[numInstance] = 0;
				firstPhase[numInstance] = 1;
				if(keepShiftList[numInstance])
				{
					backTrackCounter[numInstance]++;
					if(backTrackCounter[numInstance] > 5000)// 2*_dayCount)
					{
						backTrackCounter[numInstance] = 0;
						keepShiftList[numInstance] = 0;
					}
				}
				else
				{
				...
				for(i = 0; i < _nurseCount; i++)
					nurseInitialize(currentRoster+2*offset, shiftMap+offset, shiftList+_shiftToReroster*numInstance, nonAssignedShifts+offset, staffGaps+_dayCount*3*numInstance, localSearch, probability, i, keepShiftList[numInstance]);
				keepShiftList[numInstance] = 0;
			}
			if(firstPhase[numInstance])
			{
		...
		for(numInstance = 0; numInstance < _paralelInstanceCount; numInstance++)
		{
			...
			if(!firstPhase[numInstance])
			{
				...
				{
					infeasible[numInstance] = 1;
					if(index[numInstance] > 0){
						int swapTmp,  swapNurse;
						keepShiftList[numInstance] = 1;
						swapNurse = ((actualShifts + offset)[0])/_dayCount;
						swapTmp = shiftList[swapNurse*_dayCount+index[numInstance]];
						shiftList[swapNurse*_dayCount+index[numInstance]] = shiftList[swapNurse*_dayCount+index[numInstance]-1];
						shiftList[swapNurse*_dayCount+index[numInstance]-1] = swapTmp;
					}
					run++;
					_TESTTotalruns++;
				}
...
void nurseInitialize(unsigned int* currentRoster, unsigned int* shiftMap, unsigned int* shiftList, unsigned int* nonAssignedShift, unsigned int* staffGaps, int isLocal, float probability, int nurse, unsigned int keepShiftList)
{
	unsigned int i;
	int swap, swapIndex;
